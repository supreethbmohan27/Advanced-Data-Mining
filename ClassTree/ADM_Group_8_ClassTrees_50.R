#Class Tree with 50-50 split without Age and Menopause features
...
# Remove rows with missing values in the 'breastquad' column from the BreastCancer dataset
BreastCancer <- BreastCancer[BreastCancer$breastquad != "?", ]

# Remove rows with any missing values from the BreastCancer dataset
...
# Check CP table, when size of tree =3, the nsplit =4 and CP = 0.02500000 Prune the tree at nsplit =3 defined by the complexity parameter
#Pruned class tree
...
), col = c("brown", "red", "black", "blue", "grey", "green"), lwd = 2, cex = 0.8)
